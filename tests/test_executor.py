...
def test_run_stops_execution_after_failure() -> None:
    executor = WorkflowExecutor()

    successful_step = RecordingStep(
        StepConfig(
            key="recording",
            parameters={"value": "executed"},
        ),
        [],
    )

    failing_step = FailingStep(
        StepConfig(key="failing")
    )

    never_executed_step = NeverExecutedStep(
        StepConfig(key="never")
    )
    ...
    assert plan.steps_execution_plan[0].status == StepStatus.SUCCESS
    assert plan.steps_execution_plan[1].status == StepStatus.FAILED
    assert plan.steps_execution_plan[2].status == StepStatus.WAITING

    assert never_executed_step.executed is False


def test_run_marks_failing_step_as_failed() -> None:
    executor = WorkflowExecutor()
    ...
    plan = make_execution_plan(failing_step)
...
def test_run_does_not_raise_when_step_fails() -> None:
    executor = WorkflowExecutor()
...
def test_run_preserves_previous_successes_after_failure() -> None:
    executor = WorkflowExecutor()
...
def test_run_logs_failure(caplog) -> None:
    executor = WorkflowExecutor()
    ...
    assert "Step failing failed with error" in caplog.text
